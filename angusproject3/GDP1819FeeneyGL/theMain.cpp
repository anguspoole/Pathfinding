...
//	bool bButtonPressed = false;
	...
	// BMP STUFF
	//*********************************************
	GetBMPPixelValues("assets/resourceMap.bmp");
	CreateMap(vec_pObjectsToDraw);

	cMeshObject* gathererObj = findObjectByFriendlyName("Gatherer");

	std::vector<cMapNode*> gathererPath = Dijkstra(gathererObj);
...
//#endif 
	...
	//// ********************************************************************************
		...
		if ((gathererObj->aiType == cMeshObject::aiBehaviour::GATHER)
			&& resourceMap.size() > 0)
		{
			if (gathererObj->needPath)
			{
				gathererPath = Dijkstra(gathererObj);
				GathererPath(&gathererPath);
			}
			else
			{
				GathererPath(&gathererPath);
		...
		else if (gathererObj->aiType == cMeshObject::aiBehaviour::RETURN)
		{
			if (gathererObj->needPath)
			{
				gathererPath = AStar(gathererObj, homeNode);
				GathererPath(&gathererPath);
			}
			else
			{
		...
		else if (gathererObj->aiType == cMeshObject::aiBehaviour::IDLE)
		{
			idleInPlace(gathererObj, deltaTime, gathererObj->maxTime);
		}
