...
extern std::vector<std::vector<cMapNode*>> mapNodes;
void CreateNodeNeighbours();
void PopulateMap(std::vector<cMeshObject*> &vec_pObjectsToDraw);

extern std::map<std::string, cMapNode*> positionNodeMap;
...
extern cMapNode* homeNode;
extern std::map<std::string, cMeshObject*> resourceMap;
extern void idleInPlace(cMeshObject* obj, float dt, float maxTime);
